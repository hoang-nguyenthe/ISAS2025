...
FPS          = 30
DELTA_T      = 1 / FPS
DATA_DIR     = 'data/keypointlabel'
SUBJECT_IDS  = [1, 2, 3, 5]
OUTPUT_FILE  = 'features_continuous_unfiltered.csv'
WINDOW_SIZE  = 45
KEYPOINT_COLS_DICT = {
...
def build_features_one(df_sub):
    ...
    df_feat = pd.DataFrame(feat, index=idx)
...
def main():
    ...
    for sid in SUBJECT_IDS:
        path = os.path.join(DATA_DIR, f'keypoints_with_labels_{sid}.csv')
        df   = pd.read_csv(path)
        ...
        df['Action Label'] = df['Action Label'].replace({
            'Throwing':'Throwing things', 'Biting':'Biting nails'})

        feat_df = build_features_one(df)
        all_feat.append(feat_df)
        all_meta.append(df[['subject_id','Action Label']])
        logging.info(f'• Subject {sid} done ({len(df)} frames).')
        del df, feat_df
        gc.collect()

    final_df = pd.concat(all_meta, axis=0).reset_index(drop=True)
    features = pd.concat(all_feat,  axis=0).reset_index(drop=True)
    output   = pd.concat([final_df, features], axis=1)
