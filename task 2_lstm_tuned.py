...
import math
# Giới hạn bộ nhớ GPU (Apple M1 Metal)
...
ARTIFACT_DIR = 'final_report_saved_models/lstm_tuned'
...
def build_lstm_model(input_shape, num_classes):
    ...
    return model
def create_sequences(X_df, y_series, time_steps, step=1):
    sequences = []
    labels = []
    ...
    while start_idx < n:
        ...
        # Lấy segment
        action_segment = X_df.iloc[start_idx:end_idx].values
        ...
        if segment_length >= time_steps:
            ...
            # Trường hợp không có frame
            padded_window = np.zeros((time_steps, X_df.shape[1]))
            sequences.append(padded_window)
            labels.append(current_action)
...
if __name__ == "__main__":
    ...
    for test_subject_id in tqdm(subject_ids, desc="Tổng quan các vòng lặp LOSO"):
        ...
        gc.collect()
    print("\n===== KẾT QUẢ ĐÁNH GIÁ LOSO TRUNG BÌNH MÔ HÌNH 2 LỚP LSTM TUNED =====")
    ...
    plt.title('Confusion Matrix từ tất cả các vòng LOSO - LSTM MODEL')
